      ...
      <div class="card">
        ...
        <form @submit.prevent="createExam">
          ...
          <div class="form-row">
            ...
            <input
              type="number"
              v-model.number="examForm.passScore"
              min="0"
              max="100"
            />
          </div>
      ...
      <div class="card">
        ...
        <form @submit.prevent="createQuestion">
          ...
          <div class="form-row">
            ...
            <input
              v-model="questionForm.optionsCsv"
              placeholder="A,B,C,D"
              required
            />
          </div>
  ...
  methods: {
    async createExam() {
      ...
      try {
        let subjects = [];
        if (
          this.examForm.subjectsJson &&
          this.examForm.subjectsJson.trim().length > 0
        ) {
    ...
    async createQuestion() {
      ...
      try {
        ...
        const res = await axios.post(
          "passhub-backend-production.up.railway.app/questions",
          body
        );
        this.questionMessage = `추가 완료: ${res.data.id}`;
      } catch (err) {
        this.questionMessage = `오류: ${
          err.response?.data?.message || err.message
        }`;
      }
    },
